...
void print_char(char character, int col, int row, char attribute_byte)
{
	...
	if (!attribute_byte)
		attribute_byte = DEFAULT_STYLE;

	/*Get the video memory offset for the screen loaction		*/
	int offset = 0;

	/*If col or row are non-negative, use them for offset		*/
	if (col >= 0 && row >= 0)
...
void print_at(char *message, int col, int row)
{
	...
	int i = 0;
	while(message[i] != 0)
		print_char(message[i++], col, row, DEFAULT_STYLE);	
}
...
void clear_screen()
{
	...
	for(row = 0; row<MAX_ROWS; row++)
		for(col = 0; col < MAX_COLS; col++)
			print_char(' ', col, row, DEFAULT_STYLE);
